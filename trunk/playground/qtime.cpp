	...
	if (ZEND_NUM_ARGS() == 2){
		...
		if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,"oo|", &var_oo_0, &var_oo_1) == SUCCESS) {
			...
			if(tmp_0 == "const QString&" && tmp_1 == "Qt::DateFormat") {
				QTime *obj = (QTime) PHP_QT_FETCH();
			QTime return_object = (QTime) obj->fromString((const QString&) var_o_0, (Qt::DateFormat) var_o_1);
			zend_class_entry *ce;                                   
			object_init_ex(return_value, QTime_ce_ptr);     
			zend_rsrc_list_entry le;                            
			le.ptr = &return_object;                                       
			...
			if(tmp_0 == "const QString&" && tmp_1 == "const QString&") {
				QTime *obj = (QTime) PHP_QT_FETCH();
			QTime return_object = (QTime) obj->fromString((const QString&) var_o_0, (const QString&) var_o_1);
			zend_class_entry *ce;                                   
			object_init_ex(return_value, QTime_ce_ptr);     
			zend_rsrc_list_entry le;                            
			le.ptr = &return_object;                                       
